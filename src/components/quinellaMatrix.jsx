// components/QuinellaMatrix.jsx
// v1.0.1 - Updated with column-first highlight logic
import React from 'react';

const QuinellaMatrix = ({ quinellaOdds, horseCount = 14, highlights = [] }) => {
  // Debug: Verify input data structure
  console.log('[QuinellaMatrix] Input data sample:', {
    firstThreePairs: quinellaOdds?.slice(0, 3),
    totalPairs: quinellaOdds?.length,
    samplePair: quinellaOdds?.find(q => 
      q.horse_number_1 === "1" && 
      q.horse_number_2 === "2"
    )
  });
  ...
  const matrix = Array.from({ length: horseCount }, (_, i) => 
    Array.from({ length: horseCount }, (_, j) => {
      ...
      const pair = quinellaOdds?.find(q => {
  ...
  return (
    <div className="quinella-matrix">
      <h3>Quinella Odds Matrix</h3>
      <table>
        <thead>
          <tr>
            <th></th>
